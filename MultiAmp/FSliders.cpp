...
FSliders::~FSliders() {
	//for (size_t j = 0; j < vsl.size(); j++)		delete vsl[j];
} // /////////////////////////////////////////////////////////////////////////
...
void FSliders::activate() {
	...
	for (int j = 0; j < sz; j++) {
		CRect rctSlider(rctEdit.Width(), (int)(0.5 + top + j * heighT), rctGrp.Width() - left, (int)(0.5 + top + (j + 1.0) * heighT));
		vsl[j].slider = new CSliderCtrl();
		vsl[j].slider->Create(exampleSlider->GetStyle(), rctSlider, frame, 188999 + 1 + j);
		vsl[j].slider->SetRange((int)(fmin * kslayer), (int)(fmax * kslayer), TRUE);
		vsl[j].slider->SetPos((int)(vk->at(j) * kslayer));
