...
namespace executor{
	...
	bool LayBase::isLoad() const{
		...
		if(vv.vcpu.size() == 0) return false;
		return true;
	} // ///////////////////////////////////////////////////////////////////////////////
	string LayBase::sDumpA(const int digits)const{
		char buf[4096];
		string ret, sformat("%" + to_string(digits) + "d ");
	...
	string LayBase::sDump(const VGpuCpu<float_2>& v, const int digits)const{
		
		char buf[4096];
		string ret, sformat("%+." + to_string(digits) + "f ");
		for(int y = 0; y < sz.y; y++){
			//int y = sz.y - yr - 1;
			for(int x = 0; x < sz.x; x++){
				int idx = id(x, y);
	...
	string LayBase::sDumpF(const int digits)const{
		return sDump(vf, digits);
	} // ////////////////////////////////////////////////////////////////
	string LayBase::sDumpV(const int digits)const{
		return sDump(vv, digits);
	} // ////////////////////////////////////////////////////////////////
}