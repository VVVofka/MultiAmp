...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow){
	...
	wcex.hIcon = LoadIcon(hInstance, (LPCTSTR)IDI_MAINWND);
	wcex.hCursor = LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = NULL;
	wcex.lpszClassName = L"TutorialWindowClass";
	wcex.hIconSm = LoadIcon(wcex.hInstance, (LPCTSTR)IDI_MAINWND);
...
#endif
	ID3DBlob* pErrorBlob;
	//FILE* f; errno_t err = fopen_s(&f, "yyy.yyy", "w"); _fcloseall();
...
HRESULT InitDevice(){
	...
	hr = CompileShaderFromFile("..\\WinDxDLL\\Tutorial02.fx", "VS", "vs_4_0", &pVSBlob);
	if(FAILED(hr)){
		MessageBox(NULL,
	...
	hr = CompileShaderFromFile("..\\WinDxDLL\\Tutorial02.fx", "PS", "ps_4_0", &pPSBlob);
	if(FAILED(hr)){
		MessageBox(NULL,
					L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
		return hr;
