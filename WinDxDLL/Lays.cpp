...
bool Lays::Create(
	...
	cntMidLays = (int)fillvMidLays(sz_lay1, cfg, params);
	return isLoad();
} // //////////////////////////////////////////////////////////////////////////
bool Lays::isLoad(){
	if(lay0.isLoad() == false) return false;
...
int Lays::fillvMidLays(const int_2 sz_1, const LaysCPUCfg cfgcpu){
	...
	for(int j = 0; j < cnt_mid_lays; j++){
		...
		vMidLays[j].Create(sz_cur, &params->v[j], curtype, gpu_in);
		sz_cur /= 2;
		prevtype = curtype;
	}
	return cnt_mid_lays;
...
std::string Lays::sDumpA(int idx, const int digits)const{
	...
	if(idx > 0)
		return "a:Lay" + to_string(idx) + ": " + vMidLays[idx - 1].sDumpA(digits);

	// if(idx < 0)
	std::string ret = "a:Lay0: " + lay0.sDumpA(digits);
	for(int jmid = 0; jmid < cntMidLays; jmid++)
		ret += "a:Lay" + to_string(jmid + 1) + ": " + vMidLays[jmid].sDumpA(digits);
	return ret;
} // /////////////////////////////////////////////////////////////////////////////
std::string Lays::sDumpF(int idx, const int digits)const{
	...
	if(idx > 0)
		return "f:Lay" + to_string(idx) + ": " + vMidLays[idx - 1].sDumpF(digits);

	// if(idx < 0)
	std::string ret = "f:Lay0: " + lay0.sDumpF(digits);
	for(int jmid = 0; jmid < cntMidLays; jmid++)
		ret += "f:Lay" + to_string(jmid + 1) + ": " + vMidLays[jmid].sDumpF(digits);
	return ret;
} // /////////////////////////////////////////////////////////////////////////////
std::string Lays::sDumpV(int idx, const int digits)const{
	if(idx > cntMidLays)
		return "";
	if(idx == 0)
		return "v:Lay0: " + lay0.sDumpV(digits);
	if(idx > 0)
		return "v:Lay" + to_string(idx) + ": " + vMidLays[idx - 1].sDumpV(digits);

	// if(idx < 0)
	std::string ret = "v:Lay0: " + lay0.sDumpV(digits);
	for(int jmid = 0; jmid < cntMidLays; jmid++)
		ret += "v:Lay" + to_string(jmid + 1) + ": " + vMidLays[jmid].sDumpV(digits);
	return ret;
} // /////////////////////////////////////////////////////////////////////////////
...
std::string Lays::DumpV(int idx, const int digits)const{
	std::string s(sDumpV(idx, digits));
	_RPT0(0, s.c_str());
	return s;
} // /////////////////////////////////////////////////////////////////////////////
