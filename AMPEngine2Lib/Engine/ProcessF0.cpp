...
void ProcessF::gpuRun0(const uint_2 shift, const uint iter){
	...
	parallel_for_each(up_vgpu_a.extent,
		...
		&f_masks, shift, klayf, rSizeDn, signFor0
			...
			const int y0 = (idUp[Y] * 4 + shift.y) % dn_vgpu_a.extent[Y];
			const int idmove =
				(((signbitf(upf + f_masks[idmask + 5].x * klayf) + signFor0) / 2) << 7) |
				(sign(dn_vgpu_a[index<2>(y0, (x0 + shift.x + 3) % dn_vgpu_a.extent[X])] + 1) << 6) |

				(((signbitf(upf + f_masks[idmask + 4].x * klayf) + signFor0) / 2) << 5) |
				(sign(dn_vgpu_a[index<2>(y0, (x0 + shift.x + 2) % dn_vgpu_a.extent[X])] + 1) << 4) |

				(((signbitf(upf + f_masks[idmask + 1].x * klayf) + signFor0) / 2) << 3) |
				(sign(dn_vgpu_a[index<2>(y0, (x0 + shift.x + 1) % dn_vgpu_a.extent[X])] + 1) << 2) |

				(((signbitf(upf + f_masks[idmask].x * klayf) + signFor0) / 2) << 1) |
				(sign(dn_vgpu_a[index<2>(y0, (x0 * 4 + shift.x) % dn_vgpu_a.extent[X])] + 1));
			if(vmaskmov[idmove] & 0xb110000){
				const int src = vmaskmov[idmove] & 0xb11;
				const int dst = (vmaskmov[idmove] & 0xb1100) >> 2;
